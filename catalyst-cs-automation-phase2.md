...
| v4.10 | Phase 3.5a — Intent + Sentiment Extraction. catalyst_draft.md Step 2.5 added: Claude extracts primary_intent, secondary_intent, sentiment, escalation_risk per email before drafting. Step 6 tone calibration added (ANGRY/FRUSTRATED → empathy open, URGENT → lead with resolution, HIGH escalation_risk → ⚠️ flag after sign-off). Step 7 bq_staging.jsonl schema extended with 4 intent fields. flush_bq_staging() SQL INSERT updated to include new columns. alter_draft_log_intent.py migration run — 4 nullable STRING columns added to BigQuery draft_log. Three bug fixes applied: (1) Cleanup tool ambiguity fixed — explicit `gmail modify_email` instruction added to catalyst_cleanup.md (was guessing `gmail_modify_thread`, which doesn't exist); (2) Triage timeout bumped 15 → 20 min (14+ LLM emails hitting wall); (3) Draft batch KB read implemented — Steps 4/4.5/5 restructured so each unique KB file is read ONCE per run regardless of how many emails share it (eliminates redundant GDrive/cache calls). Draft timeout temporarily bumped 30 → 40 min for backlog clearance. *(orchestrator .py stays v4.7 — prompt system at v4.10)* |
